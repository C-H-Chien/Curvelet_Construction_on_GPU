...
__global__ void grow_edge_chains_tile_nocut_kernel(
    int num_edges,
    int slots_per_anchor,
    int bundle_cells,
    int group_max_sz,
    ...
    unsigned *dev_scratch_u)
{
    ...
    for (int f_run = 0; f_run < 2; f_run++) {
        ...
        for (int wave_base = 0; wave_base < n_active; wave_base += tile_workspaces) {
            const int wave_n = (n_active - wave_base < tile_workspaces)
                             ? (n_active - wave_base)
                             : tile_workspaces;
            ...
            for (int a = 0; a < n_active; a++) {
                ...
                if (seed_open) {
                    ...
                    else if (intersect_working_with_candidate(
                                 bundle_cells, s_work_min, s_work_max, s_tile_min, s_tile_max)) {
                        if (chain_len < group_max_sz) {
                            cand_row[chain_len++] = static_cast<unsigned>(remain_id);
                        }
                    }
            ...
            if (lane < wave_n) {
                cand_row[group_max_sz] = static_cast<unsigned>(chain_len);
                if (chain_len > 2) {
                    float *out_min = seed_working_min_frun + static_cast<size_t>(my_seed_slot) * static_cast<size_t>(bundle_cells);
                    float *out_max = seed_working_max_frun + static_cast<size_t>(my_seed_slot) * static_cast<size_t>(bundle_cells);
                    for (int b = 0; b < bundle_cells; b++) {
                        out_min[b] = s_work_min[b];
                        out_max[b] = s_work_max[b];
                    }
                }
            }
            __syncwarp();
            ...
            if (lane < wave_n) {
                ...
                if (chain_len > 2) {
                    ...
                    select_best_bundle_ks(
                        ...
                        s_work_min, s_work_max, k_max, k_min);
