...
compute_curve_bundle(edgel* ref_e, edgel* ne, double dx, double dt, double max_k, bool verbose)
{
    ...
    // CH: "Number of curves in a bundle" at the reference edgel
    //     used to initizlie sizes of the Kmin and Kmax arrays
    ...
    if ( DX2 + DY2 < 4*dx*dx ) {
        /*if (ref_e->_id == 0 && ne->_id == 9) {
            std::cout<<"IT ENTERS!!"<<std::endl;   
        }*/
        return;
    }

    // CH: else if the neighboring point is not close
    ...
    // CH > Proposition 4: Circular Arc Bundle Transport
    if ( (DX*mycos(T2)+DY*mysin(T2)) < 0 ) {

        // CH > find the max and min curvatures of the curve bundles of ref_e and ne
        for (unsigned i=0; i<_Kmax.h(); ++i) {
            for (unsigned j=0; j<_Kmax.w(); ++j) {

                // CH > get the grid location and orientation of the curve in the curve bundle w.r.t. reference edgel
                const double dx2 = Dx(i);
                const double dt2 = Dt(j);
                
                // CH > absolute grid orientation of the curve
                ...
                // CH > compute k^{\delta x1}_{min} and k^{\delta x1}_{max} from Proposition 4
                ...
                /*if (ref_e->_id == 0 && ne->_id == 7) {
                    std::cout<<"(k_dt_min, k_dt_max) = ("<<k_dt_min<<", "<<k_dt_max<<")"<<std::endl;   
                }*/
