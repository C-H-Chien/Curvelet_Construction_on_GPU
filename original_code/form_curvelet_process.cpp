...
//: Constructor
// edgel stores subpix location, ortientation, grad_mag, id of an edge

form_curvelet_process::form_curvelet_process(arrayd &edgeinfo, const unsigned &nrows, const unsigned &ncols,
                                             const double &rad, const double &gap,
...
void form_curvelet_process::build_curvelets_greedy_for_edge(edgel* eA, bool use_flag, bool forward, bool centered, bool leading)
{
    ...
    const int ii = round(eA->_pt.first);
    const int jj = round(eA->_pt.second);
    
    // CH: 7x7 neighborhood? (_rad = 3.5)
    ...
    // CH: look at the 9x9 neighborhood of the target edge? _nrad = 4
    // CH > make it as 7x7 neighborhood
    //for (int xx = ii-(int)_nrad; xx <= (int)(ii+_nrad); xx++){
    //    for (int yy = jj-(int)_nrad; yy <= (int)(jj+_nrad); yy++){
    const int reg = 3;
    for (int xx = ii-reg; xx <= (ii+reg); xx++){
        for (int yy = jj-reg; yy <= (jj+reg); yy++){
            
            // CH: width and height here are the size of raw image
            if (xx<0 || xx>=_edgemap.width() || yy<0 || yy>=_edgemap.height() )
                continue;
            
            //for all the edgels in its neighborhood
            // CH > number of edges in the _map list where subpixel locations are rounded to the same integer pixel locations
            for (unsigned k = 0; k < _edgemap._map[std::make_pair(xx, yy)].size(); k++){
                ...
                // CH > check whether the reference edgel and its neighbor edgel can form a pair
                //      if they can form a pair, construct a curve bundle and calculate max/min curvatures of each x and theta variations
                ...
                if (cm){
                    if (cm->bundle_is_valid()){ //if legal, record the hypothesis
                        ...
                        cur_hyp._eN = eB;               // < neighbor points
                        cur_hyp._cm = cm;               // < curve bundle
    ...
    for (unsigned h1=0; h1<eA_hyps.size(); h1++)
    {
        ...
        // CH > edgel_chain is a dequeue structure of edgel
            ...
            cur_cm->curve_fit_is_reasonable(cur_edgel_chain, eA, _dx))
        {
            ...
            if (debug_print) {
                ...
                for (unsigned di = 0; di < cur_edgel_chain.size(); di++) {
                    std::cout << cur_edgel_chain[di]->_id << "  ";
                }
                std::cout<<std::endl;
            }
        }
...
CC_curve_model_3d* form_curvelet_process::form_a_hypothesis(
    edgel* ref_e, edgel* e2, /*double dx, double dt,*/ 
    double max_k, bool &ref_first,  bool forward, bool centered, bool leading, bool verbose
)
{
    ...
    // CH: compute the angle of orientation from reference point to e2
    ...
    if (centered) {
        ...
        // CH > code goes to here
        // CH > CC_vurve_model constructor: constructs a curve bundle from ref_e and e2 pair
        //      which stores the max and min curvatures of the curve bundle
